...
@Injectable()
export class AboutResolve implements Resolve<any> {
    private res = new Res();
  ...
  constructor(private http: Http, private title: Title, private meta: Meta, @Inject('BASE_URL') baseUrl: string) {
    ...
    resolve(route: ActivatedRouteSnapshot,
        ...
        return forkJoin([
          ...
          new PageService(this.http, this._baseUrl).getPageBundlesByType(this.websiteLanguageId, 'eCommerceCategory', true).pipe(
                map((data: any) => {
                    return data.json();
                }),
                catchError((error: any) => of(null)))
